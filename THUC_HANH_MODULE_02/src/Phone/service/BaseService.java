...
public class BaseService {
    ...
    private static String handPath = "src/Phone/data/HandPhone.csv";
    private static String authenticPath = "src/Phone/data/AuthenticPhone.csv";
    ...
    public static boolean delete(int id, int handOrAuthentic) throws NotFoundBaseException {
        boolean check= false;
        if(handOrAuthentic== 1){
//            if(handPhonesArrayList.removeIf(e -> e.getName().equals(name))){
            if(handPhonesArrayList.removeIf(e -> e.getId()==id)){
                handPhoneFileHelper.write(handPath, handPhonesArrayList, false);
                return true;
            }
        }

        if(handOrAuthentic== 2){
//            if(authenticPhoneArrayList.removeIf(e -> e.getName().equals(name))){
            if(authenticPhoneArrayList.removeIf(e -> e.getId() == id)){
                authenticPhoneFileHelper.write(authenticPath, authenticPhoneArrayList, false);
                return true;
            }
        }
